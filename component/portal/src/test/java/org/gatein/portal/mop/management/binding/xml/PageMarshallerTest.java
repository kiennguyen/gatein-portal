...
import org.gatein.portal.mop.management.binding.xml.PageMarshaller;
import org.gatein.portal.pom.config.Utils;
import org.gatein.portal.pom.data.ApplicationData;
import org.gatein.portal.pom.data.ComponentData;
import org.gatein.portal.pom.data.ContainerData;
...
public class PageMarshallerTest extends AbstractMarshallerTest
{
   public void testHomePageUnMarshalling()
   {
      ...
      Page.PageSet pages = marshaller.unmarshal(getClass().getResourceAsStream("/org/gatein/portal/mop/management/pages-homepage.xml"));
      assertNotNull(pages);
   ...
   {
      PageMarshaller marshaller = new PageMarshaller();
      Page.PageSet pages = marshaller.unmarshal(getClass().getResourceAsStream("/org/gatein/portal/mop/management/pages-loaded.xml"));
      assertNotNull(pages);
      assertNotNull(pages.getPages());
      assertEquals(1, pages.getPages().size());
      Page page = pages.getPages().get(0);
   ...
   {
      PageMarshaller marshaller = new PageMarshaller();
      Page.PageSet pages = marshaller.unmarshal(getClass().getResourceAsStream("/org/gatein/portal/mop/management/pages-empty.xml"));
      assertNotNull(pages);
      assertNotNull(pages.getPages());
      assertEquals(1, pages.getPages().size());
      Page page = pages.getPages().get(0);
